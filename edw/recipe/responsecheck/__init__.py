...
class Recipe(object):
    ...
    def __init__(self, buildout, name, options):
        ...
        if 'start-port' not in options or not options['start-port'].isdigit():
            self.logger.error('Parameter start-port not specified or not a number.')
            raise zc.buildout.UserError('Parameter start-port not specified or not a number.')

        if 'end-port' not in options or not options['end-port'].isdigit():
            self.logger.error('Parameter end-port not specified or not a number.')
            raise zc.buildout.UserError('Parameter end-port not specified or not a number.')

        if 'step' not in options or not options['step'].isdigit():
            self.logger.error('Parameter step not specified or not a number.')
            raise zc.buildout.UserError('Parameter step not specified or not a number.')
        ...
        if 'path' not in options or not os.path.isdir(options['path']):
            self.logger.error('Parameter path not specified or directory does not exists.')
            raise zc.buildout.UserError('Parameter path not specified or directory does not exists.')
    ...
    def write_script(self):
        ...
        script.write('\tdo\n')
        curl = 'curl --write-out %{http_code} --connect-timeout 5 --silent --output /dev/null http://$backend:$i/www'
